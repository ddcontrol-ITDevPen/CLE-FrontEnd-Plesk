...
import { getDepotScheduleByDepotId, saveTemplate } from "../../../services/cleDepotScheduleService.js";
...
export function DepotOperationHours() {
    ...
    useEffect(() => {
        async function fetchExistingSettings() {
            try {
                ...
                const depotId = user.companyCode;
                setSchedule(prev => ({ ...prev, depotId: depotId }));
                const data = await getDepotScheduleByDepotId(depotId)
                if (data) {
                    setIsUpdate(true);
                ...
                if (error.response && error.response.status === 404) {
                    console.log("No existing settings found for this depot. Starting fresh.");
                    setIsUpdate(false);
                    return;
                }
                console.error("Error loading depot schedule metadata:", error);
                toast.error("Failed to load existing depot schedule configuration.");
            }
        }
