...
import { Search, Calendar, FileDown, Eye, Edit, Trash2,
...
import {
    getContainerById,
    getAleContainers,
    deleteAleContainer, updateAleContainer, getAleContainerById
...
const STATUS_CONFIG = {
    ...
    "Custom Approved": { bg: "bg-approved",text: "text-emerald-900",border: "border-teal-200" },
    "AKPS Approved": { bg: "bg-approved",text: "text-emerald-900",border: "border-teal-200" },
    "Gated-In":   { bg: "bg-gate-in-out",   text: "text-blue-900",   border: "border-indigo-200" },
    "Gated-Out":  { bg: "bg-gate-in-out", text: "text-green-900", border: "border-green-200" },
    "Custom Rejected":  { bg: "bg-red-100",    text: "text-red-900",    border: "border-red-200" },
    "AKPS Rejected":  { bg: "bg-red-100",    text: "text-red-900",    border: "border-red-200" }
...
export function TerminalDashboard() {
    ...
    const filteredContainers = useMemo(() => {
            let result = containers.filter(cont => {
                ...
                // 1. Handle "All" filter (Default view)
                if (filterStatus === "All") {
                    matchesStatus = cont.status === "Enroute";
                }
                // 2. Handle "Custom Approved" filter
                else if (filterStatus === "Custom Approved") {
                    matchesStatus = cont.customStatus?.trim().toLowerCase() === "approved";
                }
                // 3. Handle "AKPS Approved" filter
                else if (filterStatus === "AKPS Approved") {
                    matchesStatus = cont.akpsStatus?.trim().toLowerCase() === "approved";
                }
                // 4. Handle Rejected filters
                else if (filterStatus === "Custom Rejected") {
                    matchesStatus = cont.customStatus?.trim().toLowerCase() === "rejected";
                }
                else if (filterStatus === "AKPS Rejected") {
                    matchesStatus = cont.akpsStatus?.trim().toLowerCase() === "rejected";
                }
                // 5. Handle standard statuses (Assigned, Enroute, Gated-In, etc.)
                else {
        ...
        <Layout role="terminal">
            ...
            <div className="space-y-6">
                <div className="flex flex-col gap-0">
                    <h1 className="text-2xl font-bold">Terminal - Pre Arrival</h1>
                    <p className="text-gray-500 text-sm">Manage all your Pre-Arrival here</p>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-x-auto">
                    <table className="w-full text-left border-collapse lg:table-auto">
                        ...
                        <tr>
                            ...
                            {/* Add these after the Status column header */}
                            <th className="p-4 border-b w-30 text-center">
                        ...
                        {isLoading ? (
                            <tr>
                                ...
                                filteredContainers.map((cont, index) => {
                                    ...
                                    console.log("Container Keys:", Object.keys(cont));
        ...
        {cont.akpsStatus || "-"}
                                            ...
                                            <td className="p-4">
                                                ...
                                                <div className="flex items-center justify-center gap-3">
                                                    {/* Only display Clock icons if status is Enroute AND BOTH Custom and AKPS are approved */}
                                                    {cont.status === "Enroute" &&
                                                        cont.customStatus?.toLowerCase() === "approved" &&
                                                        cont.akpsStatus?.toLowerCase() === "approved" && (
                                                            <>
                                                                <Clock
                                                                    size={18}
                                                                    className="text-emerald-600 cursor-pointer hover:text-emerald-800"
                                                                    title="Gate In"
                                                                    onClick={() => setStatusModal({ isOpen: true, id: cont.containerId, nextStatus: "Gated-In" })}
                                                                />
                                                    ...
                                                    <Eye
                                                        ...
                                                        onClick={() => navigate(`/terminal/view/${cont.containerId}`)}
                                                    />
